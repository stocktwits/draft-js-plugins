...
var addMention = function addMention(editorState, mention, mentionPrefix, mentionTrigger, entityMutability) {
  ...
  var mentionReplacedContent = draftJs.Modifier.replaceText(editorState.getCurrentContent(), mentionTextSelection, "".concat(mentionPrefix).concat(mention.name), null, // no inline style needed
...
function (_Component) {
  ...
  function MentionSuggestions() {
    ...
    _defineProperty(_assertThisInitialized(_this), "onSpacebar", function (keyboardEvent) {
      var matchingSuggestion = find(_this.props.suggestions, function (suggestion) {
        return _this.props.matchSuggestion(suggestion, _this.lastSearchValue);
      });

      if (matchingSuggestion) {
        keyboardEvent.preventDefault();

        _this.onMentionSelect(matchingSuggestion);
      }
    });
    ...
    _defineProperty(_assertThisInitialized(_this), "openDropdown", function () {
      ...
      _this.props.callbacks.keyBindingFn = function (keyboardEvent) {
        ...
        if (keyboardEvent.keyCode === 32) {
          _this.onSpacebar(keyboardEvent);
        }
      };
